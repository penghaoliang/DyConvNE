    ...
    def get_neg_ent(self, triple, label):
        def get(triple, label):
            if self.strategy == 'one_to_x':
                pos_obj = triple[2]
                if self.c:
                    l = self.c[tuple(triple.numpy())]
                    if pos_obj.item() in l:
                        l.remove(pos_obj.item())
                    neg_ent1 = np.int32(np.random.choice(l, self.p.n_num, replace=False)).reshape([-1])
                    mask = np.ones([self.p.num_ent], dtype=np.bool)
                    mask[label] = 0
                    neg_ent = np.int32(np.random.choice(self.entities[mask], self.p.neg_num-len(neg_ent1), replace=False)).reshape([-1])
                    neg_ent = np.concatenate((pos_obj.reshape([-1]), neg_ent, neg_ent1))
                else:
                    mask = np.ones([self.p.num_ent], dtype=np.bool)
                    mask[label] = 0
