...
class Main(object):
	...
	def load_data(self):
		...
		if self.p.pre_emb:
			self.entity_embeddings = np.loadtxt('./data/{}/{}.txt'.format(self.p.dataset, 'entity2vec200'),dtype=np.float64)
			self.relation_embeddings = np.loadtxt('./data/{}/{}.txt'.format(self.p.dataset, 'relation2vec200'), dtype=np.float64)
			self.relation_embeddings = np.tile(self.relation_embeddings, (2, 1))
			print("Initialised relations and entities")
		else:
		...
		self.data_iter = {
			...
			'train_head': get_data_loader(TestDataset, 'train_head', self.p.batch_size),
			'train_tail': get_data_loader(TestDataset, 'train_tail', self.p.batch_size),
	...
	def add_model(self):
		...
		"""
		model = InteractE(self.p, self.chequer_perm, self.entity_embeddings, self.relation_embeddings, self.device)
	...
	def evaluate(self, split, epoch=0):
		...
		self.logger.info(
			'[Epoch {} {}]: MRR: Tail : {:.5}, Head : {:.5}, Avg : {:.5} hit@1:{} hit@10:{}'.format(epoch, split,
																								   results['left_mrr'],
																								   results['right_mrr'],
																								   results['mrr'],
																								   results['hits@1'],
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				if split == 'train':
					r = torch.argsort(pred, dim=1, descending=True)[:, self.p.hn_s:self.p.hn_e]
					for i in range(len(sub)):
						self.tra_neg[(sub[i].item(), rel[i].item(), obj[i].item())] = list(r[i].cpu().numpy())
				ranks 			= ranks.float()
				...
				a = results['hits@1'] / (self.p.batch_size * (step + 1)) * 100
				b = results['hits@10'] / (self.p.batch_size * (step + 1)) * 100
				if step == 1:
					self.logger.info(
						'[{}, {} Step {}],hits@1:{},hits@10:{}\t{}'.format(split.title(), mode.title(), step, a, b, self.p.name))
	...
	def run_epoch(self, epoch):
		...
		for step, batch in enumerate(train_iter):
			...
			if step % 10 == 0:
				self.logger.info('[E:{}| {}]: Train Loss:{:.5},  Train MRR: {:.5}, Valid MRR: {:.5}, Best Valid MRR: {:.5}, \t{}'.format(epoch, step, loss,self.tra_mrr,self.val_mrr ,self.best_val_mrr, self.p.name))
	...
	def fit(self):
		...
		scheduler = torch.optim.lr_scheduler.StepLR(
			self.optimizer, step_size=150, gamma=0.5, last_epoch=-1)
		...
		for epoch in range(self.p.max_epochs):
			...
			scheduler.step()
			train_result = self.evaluate('train', epoch)
			self.data_iter['train'].dataset.c = self.tra_neg

			val_result	= self.evaluate('test', epoch)
			self.train_results.append(train_result['mrr'])
			self.val_results.append(val_result['mrr'])
			self.val_mrr = val_result['mrr']
			self.tra_mrr = train_result['mrr']
			if val_result['mrr'] > self.best_val_mrr:
				self.best_val		= val_result
				self.best_val_mrr	= val_result['mrr']
				self.best_epoch		= epoch
				self.save_model(save_path)
			self.logger.info('[Epoch {}]:  Training Loss: {:.5},  Train MRR: {:.5}, Valid MRR: {:.5}, Best Valid MRR: {:.5}, \n\n\n'.format(epoch, train_loss, train_result['mrr'], val_result['mrr'], self.best_val_mrr))
...
if __name__ == "__main__":
	...
	parser.add_argument("--pre_emb", type=bool, default=True, help="Use pretrained embeddings")
	# Model parameters
	...
	parser.add_argument('--num_filt',  	dest="num_filt",      	default=100,     	type=int,       	help='Number of filters in convolution')
	parser.add_argument('--ker_sz',    	dest="ker_sz",        	default=3,     		type=int,       	help='Kernel size to use')
	parser.add_argument('--perm',      	dest="perm",          	default=3,      	type=int,       	help='Number of Feature rearrangement to use')
	...
	parser.add_argument('--hn_s', dest="hn_s", default=30, help='Hard negative sampling start at ')
	parser.add_argument('--hn_e', dest="hn_e", default=100, help='Hard negative sampling end at')
	parser.add_argument('--n_num', dest="n_num", default=50, help='Negative sampling number')
